...
if (.not.diag_wan_ibt) then
  ...
  allocate(vtrc(3,nwann))
  do n=1,nwann
    j=idxmegqwan(n,n,0,0,0)
    vtrc(:,n)=avec(:,1)*imegqwan(3,j)+avec(:,2)*imegqwan(4,j)+&
      avec(:,3)*imegqwan(5,j)
  enddo
  allocate(zm1(nwann,1))
  allocate(expkqt(nwann))
  do ikloc=1,nkptnrloc
    ik=mpi_grid_map(nkptnr,dim_k,loc=ikloc)
    do n=1,nwann
      expkqt(n)=exp(dcmplx(0.d0,-dot_product(vkcnr(:,ik)+vqc(:,iq),vtrc(:,n))))
    enddo
    do i1=1,nmegqblhwan(ikloc)
      i=imegqblhwan(i1,ikloc)
      ibloc=mpi_grid_map(nmegqblh(ikloc),dim_b,glob=i,x=xloc)
