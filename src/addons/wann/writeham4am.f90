...
integer isp, hdim, ntype, l, norb, itype, istart
character*1      xx
logical lpmat
complex(8), allocatable :: zm(:,:)
real(8), allocatable :: dm(:,:),eval(:)
integer values(8), seconds, hash
integer, allocatable   :: basis_desc(:,:)
real(8)  reh, imh, tmp, sc, sc1, sc2, sc3, alat(3,3)
...
!  do i = 1,3
!    do j = 1,3
!      if (ABS(avec(i,j)) /= 0 .and. tmp < ABS(avec(i,j))) tmp = ABS(avec(i,j))
!    end do
!  end do
  write(200,'(a5)') '&cell'
...
!  end do
 ...
 write(200,*)
! do i=1,nwantot !ntype
!   write(200,'(7i4)')wan_info(wi_atom,i),wan_info(wi_lm,i),wan_info(wi_spin,i),wan_info(wi_type,i),wan_info(wi_group,i),wan_info(wi_idxa,i),wan_info(wi_idxo,i)
! enddo
