...
subroutine d_reduce_cart2(comm,all,val,n,op)
use modmain
#ifdef _MPI_
use mpi
#endif
...
if (all) then
  allocate(tmp(n))
  call mpi_allreduce(val,tmp,n,MPI_DOUBLE_PRECISION,op,comm,ierr)
  val=tmp
  deallocate(tmp)
else
  if (rank.eq.root) allocate(tmp(n))
  call mpi_reduce(val,tmp,n,MPI_DOUBLE_PRECISION,op,root,comm,ierr)
  if (rank.eq.root) then
    val=tmp
    deallocate(tmp)
  endif
...
subroutine i_reduce_cart(comm,all,val,n)
use modmain
#ifdef _MPI_
use mpi
#endif
...
if (all) then
  allocate(tmp(n))
  call mpi_allreduce(val,tmp,n,MPI_INTEGER,MPI_SUM,comm,ierr)
  val=tmp
  deallocate(tmp)
else
  if (rank.eq.root) allocate(tmp(n))
  call mpi_reduce(val,tmp,n,MPI_INTEGER,MPI_SUM,root,comm,ierr)
  if (rank.eq.root) then
    val=tmp
    deallocate(tmp)
  endif
