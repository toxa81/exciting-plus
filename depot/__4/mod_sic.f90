...
subroutine s_get_wanval2(x,wanval,itp,ir,rmax,rcutoff)
use modmain
use mod_nrkp
use mod_wannier
use mod_madness
...
subroutine s_spinor_2func_val(x,f1lm,f2lm,zval1,zval2,rmax,rcutoff)
use modmain
implicit none
! arguments
real(8), intent(in) :: x(3)
...
if (present(rmax)) then
  if (sum(x(:)**2).gt.(rmax**2+1d-8)) then
    write(*,'("Error(s_spinor_2func_val) outside of rmax")')
    write(*,'("  rmax : ",G18.10)')rmax
    write(*,'("   x   : ",3G18.10)')x
