...
integer ist,jst,i,j,k,l,lm,nm,n,itype
...
if (wannier.and.wann_add_poco) then
  allocate(wannfv(nstfv,wann_nmax,wann_nspin))
  wannfv=zzero
endif
...
! compute the first-variational wavefunctions
    ...
    if (wannier.and.wann_add_poco) then
      do ist=1,nstfv
        do ispn=1,nspinor
          do n=1,nwann(ispn)
            wannfv(ist,n,ispn)=wannfv(ist,n,ispn)+ &
              zfmtinp(.true.,lmaxvr,nrcmt(is),rcmt(:,is),lmmaxvr,wfmt1(:,:,ist),wannmt(:,:,ias,ispn,n,ik))
          enddo
        enddo
      enddo
    endif
...
! Fourier transform wavefunction to real-space
    ...
    if (wannier.and.wann_add_poco) then
      do ispn=1,nspinor
        do n=1,nwann(ispn)
          do ir=1,ngrtot
            wannfv(jst,n,ispn)=wannfv(jst,n,ispn)+ &
            cfunir(ir)*dconjg(zfft1(ir))*wannit(ir,ispn,n,ik)*omega/dble(ngrtot)
          enddo
        enddo
      enddo
    endif
    ...
    do jst=1,nstfv
      do ispn=1,nspinor
        ...
        do n=1,nwann(ispn)
          ...
          zt1=zt1+wannfv(ist,n,ispn)*dconjg(wannfv(jst,n,ispn))*wann_v(itype)
        enddo
        evecsv(ist+(ispn-1)*nstfv,jst+(ispn-1)*nstfv)=&
          evecsv(ist+(ispn-1)*nstfv,jst+(ispn-1)*nstfv)+zt1
      enddo
    enddo
  enddo
endif
...
if (wannier.and.wann_add_poco) deallocate(wannfv)
