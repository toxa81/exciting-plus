...
! !ROUTINE: zfmtinp
! !INTERFACE:
complex(8) function zfmtinp_(tsh,lmax,nr,r,ld,zfmt1,zfmt2)
...
do ir=1,nr
  t1=r(ir)**2
  zf1(ir)=t1*zdotc(lmmax,zfmt1(:,ir),1,zfmt2(:,ir),1)
end do
zt1=dcmplx(0.d0,0.d0)
do ir=1,nr-1
  zt1=zt1+0.5d0*(zf1(ir)+zf1(ir+1))*(r(ir+1)-r(ir))
enddo
zfmtinp_=zt1
...
if (.not.tsh) zfmtinp_=zfmtinp_*fourpi/dble(lmmax)
return
end function
!EOC
