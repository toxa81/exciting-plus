...
! !DESCRIPTION:
!   Calculate the Slater parameters for LDA+U calculation with different
...
! external functions
real(8) fluykwa,fluykwa0
external fluykwa,fluykwa0
...
e(:)=elu(:,is)
lambda=lambdalu(is)
if (inptypelu.lt.4) then
! F(0) = U for any l-shell
  if (inptypelu.eq.1) f(0)=u
  select case(l)
  case(0)
! s electrons only f(0)=u
    if (inptypelu.eq.3) then
      f(0)=e(0)
      u=f(0)
    end if
  case(1)
...
! Slater parameters, Eq. 115-117, LN Notes 29-12-08
  ...
  case default
    write(*,*)
    write(*,'("Error(genflu): invalid l : ",I8)') l
    write(*,*)
    stop
  end select
else if (inptypelu.ge.4) then
...
! calculate radial functions for Slater parameters
  call genflufr
  if (inptypelu.eq.5) then
    ufix=ulufix(is)
! calculate the lambda corresponding to ulufix
! lambdalu0 is in/out and it is initialized to 0 in readinput
    call findlambdalu(is,l,ufix,lambdalu0(is),lambda)
  end if
  do q=0,l
    k=2*q
...
! screened Slater parameters
      f(k)=fluykwa(is,l,k,lambda)
    end if
  end do
end if
! calculate U and J from Slater integrals
if (inptypelu.ne.1) then
  u=f(0)
  select case(l)
  case(0)
    j=0.d0
