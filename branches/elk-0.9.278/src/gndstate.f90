...
if ((task.eq.1).or.(task.eq.3)) then
  ...
  call poteff
  call genveffig
  if (cart_root()) write(60,'("Density and potential initialised from atomic data")')
end if
if (cart_root()) call flushifc(60)
...
if (cart_root().and.((task.eq.0).or.(task.eq.2))) call delevec
...
! write the eigenvalues/vectors to file
  ...
  call cart_reduce(evalsv(1,1),nstsv*nkpt,dims=(/dim_nkpt/),side=.true.)
  if (cart_root())  then
...
! write the Fermi energy to file
  ...
  call cart_bcast(occsv(1,1),nstsv*nkpt)
...
! add to the density and magnetisation
  ...
  call cart_reduce(rhomt(1,1,1),lmmaxvr*nrmtmax*natmtot,&
    dims=(/dim_nkpt/),side=.true.,all=.true.)
  call cart_reduce(rhoir(1),ngrtot,dims=(/dim_nkpt/),&
    side=.true.,all=.true.)
  if (spinpol) then
    call cart_reduce(magmt(1,1,1,1),lmmaxvr*nrmtmax*natmtot*ndmag,&
      dims=(/dim_nkpt/),side=.true.,all=.true.)
    call cart_reduce(magir(1,1),ngrtot*ndmag,dims=(/dim_nkpt/),&
      side=.true.,all=.true.)
  endif
