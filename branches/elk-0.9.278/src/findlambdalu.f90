...
!BOP
! !ROUTINE: findlambdalu
! !INTERFACE:
subroutine findlambdalu(is,l,ufix,lambda0,lambda)
...
!   corresponding to $U\sim 0$ (that perfectly mimics a bare LDA calculation).
...
if (ufix.lt.umin) then
  lambda=lambdamax
  write(*,'("Info(findlambdalu): lambdalu set to lambdamax (",G18.10,")")') &
   lambdamax
  return
end if
...
! unscreened Slater parameters
    f=fluykwa0(is,l,0)-ufix
  else
! screened Slater parameters
    f=fluykwa(is,l,0,lambda)-ufix
  end if
...
! unscreened Slater parameters
  fold=fluykwa0(is,l,0)-ufix
else
! screened Slater parameters
  fold=fluykwa(is,l,0,lambdaold)-ufix
end if
...
if (nit.ge.maxit) then
  write(*,*)
  write(*,'("Error(findlambdalu): max number of iterations to obtain &
   &lambdalu reached")')
  write(*,*)
  stop
else
