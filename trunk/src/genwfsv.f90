...
subroutine genwfsvj(tocc,ngp,igpig,evalsvp,apwalm,evecfv,evecsv,wfmt,wfir,j)
! !USES:
use modmain
! !INPUT/OUTPUT PARAMETERS:
!   tocc    : .true. if only occupied wavefunctions are required (in,logical)
...
complex(8), intent(in) :: evecfv(nmatmax,nstfv)
complex(8), intent(in) :: evecsv(nstsv,nstsv)
complex(8), intent(out) :: wfmt(lmmaxvr,nrcmtmax,natmtot,nspinor)
complex(8), intent(out) :: wfir(ngrtot,nspinor)
integer, intent(in) :: j
! local variables
integer ispn,is,ia,ias
integer i,n,ist,igp,ifg
real(8) t1
complex(8) zt1
! allocatable arrays
logical, allocatable :: done(:)
...
! add to spinor wavefunction
                call zaxpy(n,zt1,wfmt2(1,1,ist),1,wfmt(1,1,ias,ispn),1)
              end if
! end loop over first-variational states
            end do
! end loop over spin
...
! convert from spherical harmonics to spherical coordinates
          call zgemm('N','N',lmmaxvr,nrcmt(is),lmmaxvr,zone,zbshtapw,lmmaxapw, &
           wfmt1,lmmaxvr,zzero,wfmt(1,1,ias,1),lmmaxvr)
        end if
      end if
! end loops over atoms and species
  end do
      ...
      do ispn=1,nspinor
        do ist=1,nstfv
          ...
          if (abs(dble(zt1))+abs(aimag(zt1)).gt.epsocc) then
            ...
            do igp=1,ngp
              ifg=igfft(igpig(igp))
              wfir(ifg,ispn)=wfir(ifg,ispn)+zt1*evecfv(igp,ist)
            end do
          end if
        end do
      end do
...
! spin-unpolarised wavefunction
      do igp=1,ngp
        ifg=igfft(igpig(igp))
        wfir(ifg,1)=t1*evecfv(igp,j)
      end do
    end if
! Fourier transform wavefunction to real-space
    do ispn=1,nspinor
      call zfftifc(3,ngrid,1,wfir(1,ispn))
    end do
  end if
