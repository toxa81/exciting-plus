                                      ...
                                      double *occsv_, double *densmt_, double *densir_, int32_t *flags_)
{
    ...
    lapw_eigen_data eigen_data(&p.kpoints[ikloc]);
    eigen_data.pack_apwalm(apwalm_);
    
    if (flags & 1) lapw_gev<lapw_impl,second_variational>(eigen_data);
    
    
    /*tensor<complex16,2> h(msize, msize);
    tensor<complex16,2> o(msize, msize);
    
    mdarray<lapw_seceqn_impl,complex16,2> h_new(NULL, msize, msize);
    h_new.allocate();
    mdarray<lapw_seceqn_impl,complex16,2> o_new(NULL, msize, msize);
    o_new.allocate();
