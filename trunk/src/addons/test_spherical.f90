...
!allocate(ylm(lmmaxwan))
!allocate(rlmb(ntp,lmmaxwan))
!allocate(ylmb(ntp,lmmaxwan))
!! Lebedev mesh
!allocate(spx(3,ntp))
!call leblaik(ntp,spx,wtp)
!! get (theta,phi) of each spx vector
!do itp=1,ntp                   
...
!  call genylm(lmaxwan,tp(1,itp),ylm)  
!  do lm=1,lmmaxwan
!    rlmb(itp,lm)=rlm(lm)*wtp(itp)
!    ylmb(itp,lm)=ylm(lm)*wtp(itp)
!  enddo
!enddo
!!deallocate(wtp,tp)
!
...
!  do itp=1,ntp
!    vrc(:)=spx(:,itp)*s_r(ir)
!    call s_gen_wan(1,vrc,wanval)
!    s_wantp(itp,ir)=wanval(1)
!  enddo
!enddo
!call zgemm('T','N',lmmaxwan,s_nr,ntp,zone,ylmb,ntp,s_wantp,ntp,zzero,&
!  s_wanlm,lmmaxwan)
! 
...
!do lm=1,lmmaxwan
!  do ir=1,s_nr-1
!    d1=d1+0.5d0*(abs(s_wanlm(lm,ir))**2*s_r(ir)**2+abs(s_wanlm(lm,ir+1))**2*s_r(ir+1)**2)*(s_r(ir+1)-s_r(ir))
!  enddo
!enddo
!
!write(*,*)"norm : ",d1
