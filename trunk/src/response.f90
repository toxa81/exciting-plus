...
real(8)                 :: vkq0l(3),t1
integer                 :: ik1,ik2,ist1,ist2,ik,jk,jkq,ig,ir,is,i,j,i1,i2,i3,ig1,ig2,ig3,ie
...
!              3: index of symmetry operation for k-point
!              4: index of equivalent k'-point in irreducible part of BZ
!              5: index of symmetry operation for k'-point
...
! for parallel execution
    if (ismpi) then
      call findkpt(vklnr(1,ik),ikq(ik,3),ikq(ik,2))
      call findkpt(vklnr(1,ikq(ik,1)),ikq(ik,5),ikq(ik,4))
    endif
  enddo
...
! find indexes of k-points to send and receive
    ...
    do ik=1,nkptloc(0)
	  ...
	  if (ik.le.nkptloc(i)) then
	    tag=(ik*nproc+i)*10
	    jk=isend(ik,i,1)
	    call mpi_send(evecfv0(1,1,1,jk),nmatmax*nstfv*nspnfv,MPI_COMPLEX,i,tag,MPI_COMM_WORLD,ierr)
	    tag=tag+1
	    call mpi_send(evecsv0(1,1,jk),nstsv*nstsv,MPI_COMPLEX,i,tag,MPI_COMM_WORLD,ierr)
	    tag=tag+1
	    jk=isend(ik,i,2)
	    call mpi_send(evecfv0(1,1,1,jk),nmatmax*nstfv*nspnfv,MPI_COMPLEX,i,tag,MPI_COMM_WORLD,ierr)
	    tag=tag+1
	...
	evecsv2(:,:)=evecsv0(:,:,jk)
	  ...
	  call mpi_recv(evecfv1,nmatmax*nstfv*nspnfv,MPI_COMPLEX,0,tag,MPI_COMM_WORLD,status,ierr)
	  tag=tag+1
	  call mpi_recv(evecsv1,nstsv*nstsv,MPI_COMPLEX,0,tag,MPI_COMM_WORLD,status,ierr)
	  tag=tag+1
	  call mpi_recv(evecfv2,nmatmax*nstfv*nspnfv,MPI_COMPLEX,0,tag,MPI_COMM_WORLD,status,ierr)
	  tag=tag+1
	  call mpi_recv(evecsv2,nstsv*nstsv,MPI_COMPLEX,0,tag,MPI_COMM_WORLD,status,ierr)
	endif
      endif
...
! generate wave-functions at k
        call getevecfvp(vklnr(1,jk),vgklnr(:,:,jk),evecfv1)
...
! generate wave-functions at k'=k+q-K
        call getevecfvp(vklnr(1,jkq),vgklnr(1,1,jkq),evecfv2)
...
! get eigen-values  
  ...
  do ik=1,nkptnr
    ...
    do i=1,num_nnp(ik)
      do ig1=1,ngvec_chi
        do ig2=1,ngvec_chi
	  mtrx1(ig1,ig2)=zrhofc(ig1,i)*dconjg(zrhofc(ig2,i))
	enddo
      enddo
      do ie=1,nepts
	...
	call zaxpy(ngvec_chi**2,wt,mtrx1,1,chi0(1,1,ie),1)
      enddo !ie
    enddo !i
  enddo !ik
  close(160)
