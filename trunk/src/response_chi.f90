...
! find number of G-vectors by given number of G-shells
  ...
  if (iproc.eq.0) then
    allocate(zrhofcp1(ngvec_me,max_num_nnp,nkptnr)
  endif
...
! generate G+q vectors  
    ...
    gq0(ig)=sqrt(vgq0c(1,ig)**2+vgq0c(2,ig)**2+vgq0c(3,ig)**2)
    vc(ig)=fourpi/gq0(ig)**2 
    if (iproc.eq.0) write(150,'(I4,2x,2G18.10)')ig,gq0(ig),vc(ig)
  enddo
...
! get eigen-values
  ...
  if (iproc.eq.0) then
    do ik=1,nkptnr
      ...
      read(160)zrhofcp1(1:ngvec_me,1:num_nnp(ik),ik)
    enddo
    close(160)
  endif
#ifdef _MPI_
  ...
  if (iproc.eq.0) then
    do i=1,nproc-1
      tag=i
      call mpi_send(zrhofcp1(1,1,ikptloc(i,1)),ngvec_me*max_num_nnp*nkptloc(i),MPI_DOUBLE_COMPLEX,i,tag,MPI_COMM_WORLD,ierr)
    enddo
    zrhofcp(:,:,1:nkptloc(0))=zrhofcp1(:,:,1:nkptloc(0))
  else
    tag=iproc
    call mpi_recv(zrhofcp,ngvec_me*max_num_nnp*nkptloc(iproc),MPI_DOUBLE_COMPLEX,0,tag,MPI_COMM_WORLD,status,ierr)
  endif
#endif
  
  chi0=dcmplx(0.d0,0.d0)
  do ik=1,nkptloc(0)
    if (iproc.eq.0) write(150,*)'ik=',ik,' out of ',nkptnr
    if (ik.le.nkptloc(iproc)) then
      ...
      do i=1,num_nnp(jk)
        do ig1=1,ngvec_chi
          do ig2=1,ngvec_chi
	    mtrx1(ig1,ig2)=zrhofcp(ig1,i,ik)*dconjg(zrhofcp(ig2,i,ik))
	  enddo
        enddo
        do ie=1,nepts
          wt=docc(jk,i)/(evalsvnr(nnp(jk,i,1),ik)-evalsvnr(nnp(ik,i,2),jk)+w(ie))
...
complex(8) ,allocatable :: zrhofcp1(:,:,:)
